...
/**
 * Taken from Essentials. <b>All</b> credit to those guys
 */
public class FormatUtil {
    ...
    static final transient Pattern LOGCOLOR_PATTERN = Pattern
            .compile("\\x1B\\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]");
    static final transient Pattern URL_PATTERN = Pattern
            .compile("((?:(?:https?)://)?[\\w-_\\.]{2,})\\.([a-zA-Z]{2,3}(?:/\\S+)?)");
    ...
    // This is the general permission sensitive message format function, checks
    // for urls.
    public static String formatMessage(final String input) {
        if (input == null) {
            return null;
        }
        final String message = formatString(input);
        return message;
    }

    // This method is used to simply replace the ess colour codes with minecraft
    // ones, ie &c
    ...
    // This is the general permission sensitive message format function, does
    // not touch urls.
