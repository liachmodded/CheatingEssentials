...
public class GlyphCache {
     ...
     */
    private final BufferedImage glyphCacheImage = new BufferedImage(TEXTURE_WIDTH, TEXTURE_HEIGHT,
    ...
    private final IntBuffer imageBuffer = ByteBuffer
            .allocateDirect(4 * TEXTURE_WIDTH * TEXTURE_HEIGHT).order(ByteOrder.BIG_ENDIAN)
            .asIntBuffer();
    /**
     ...
     */
    GlyphVector layoutGlyphVector(final Font font, final char text[], final int start,
     ...
     * @param start the offset into text at which to start checking characters for
     *              being supported by a font
     * @param limit the (offset + length) at which to stop checking characters
     * @param style a combination of the Font.PLAIN, Font.BOLD, and Font.ITALIC to
     ...
     *
     * @param font      the font to which this glyphCode belongs and which was used to
     ...
     */
    void cacheGlyphs(final Font font, final char text[], final int start, final int limit,
        ...
        for (int index = 0; index < numGlyphs; index++) {
			 ...
			 */
            final Rectangle rect = vector.getGlyphPixelBounds(index, null, -vectorBounds.x,
                    -vectorBounds.y);
            ...
            glyphCacheGraphics.drawImage(stringImage, cachePosX, cachePosY, cachePosX + rect.width,
                    cachePosY + rect.height, rect.x, rect.y, rect.x + rect.width, rect.y
                            + rect.height, null);
     ...
     *
     * @param dirty The rectangular region in glyphCacheImage that has changed and
    ...
    private void updateTexture(final Rectangle dirty) {
        ...
        if (dirty != null) {
            ...
            GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, dirty.x, dirty.y, dirty.width,
                    dirty.height, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, imageBuffer);
        }
    }
